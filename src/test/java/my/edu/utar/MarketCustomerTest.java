...
public class MarketCustomerTest {
    ...
    @Test
    @Parameters({"false,false,true,15", "false,true,false,10", "true,true,false,30", "true,false,true,20", "true,false,false,20"})
    public void testProcessDiscountValidValues(boolean haveCoupun, boolean existAcc, boolean newAcc, int expectedTotalDiscount) {
       customer.processDiscount(haveCoupun, existAcc, newAcc);
       assertEquals(expectedTotalDiscount, customer.getTotalDiscount());
    }

    @Test(expected = IllegalArgumentException.class)
    @Parameters({"false,true,true", "true,true,true", "false,false,false"})
